...
uint32 addr_lo ( void * ptr) 
{
    return (uint32_t)(((uint64_t)ptr) & 0xFFFFFFFF)
}


uint32 addr_hi ( void * ptr) 
{
...
int main(int argc, char *argv[])
{
    ...
    unsigned int *i1_buff = NULL, *i2_buff = NULL;
    ...
    ocaccel_action_flag_t action_irq = 0;
    ...
    i1_buff = ocaccel_malloc(size * sizeof(unsigned int));
    i2_buff = ocaccel_malloc(size * sizeof(unsigned int));
    result_buff = ocaccel_malloc(size * sizeof(unsigned int));
    verify_buff = ocaccel_malloc(size * sizeof(unsigned int));

    printf("============================\n");
    printf("i1_buff address = %p\n", i1_buff);
    printf("i2_buff address = %p\n", i2_buff);
    ...
    for (i = 0; i < size; i++) {
        i1_buff[i] = i;     //Give a simple number for easier debugging.
        i2_buff[i] = i*2;   //Give a simple number for easier debugging.
        result_buff[i] = 0; //Wait FPGA to calculate
        verify_buff[i] = i1_buff[i] + i2_buff[i];
    }
