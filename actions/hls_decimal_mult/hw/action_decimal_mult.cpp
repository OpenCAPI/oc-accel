...
static void mbus_to_decimal(snap_membus_512_t *data_read, mat_elmt_t *table_decimal_in)
{
	union {
		uint64_t     value_u;
		mat_elmt_t   value_d;
...
static void  decimal_to_mbus(mat_elmt_t *table_decimal_out, snap_membus_512_t *data_to_be_written)
{
	union {
		mat_elmt_t   value_d;
		uint64_t     value_u;
			  ...
			  action_reg *act_reg)
{
	...
	memcpy(buffer_in, (snap_membus_512_t*)(din_gmem + i_idx), MAX_NB_OF_DECIMAL_READ*sizeof(mat_elmt_t));
