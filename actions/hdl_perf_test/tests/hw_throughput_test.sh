...
function usage() {
    ...
    echo "  test throughput of oc-accel bridge mode widh hdl_perf_test:"
...
function test_perf_test {
    ...
    cmd="hdl_perf_test -C${ocaccel_card} -c 100 -w 0x00000601 -n ${num} -N 0 -p 0x00001F07 -t 100000  >> hdl_perf_test.log 2>&1"
    eval ${cmd}
    if [ $? -ne 0 ]; then
        echo "cmd: ${cmd}"
        echo "failed, please check hdl_perf_test.log"
        exit 1
    fi
    echo "ok"

    echo -n "Write from FPGA to Host Memory ... "
    cmd="hdl_perf_test -C${ocaccel_card} -c 100 -w 0x00000601 -n 0 -N ${num} -P 0x00001F07 -t 100000  >> hdl_perf_test.log 2>&1"
    eval ${cmd}
    if [ $? -ne 0 ]; then
        echo "cmd: ${cmd}"
        echo "failed, please check hdl_perf_test.log"
    ...
    cmd="hdl_perf_test -C${ocaccel_card} -c 100 -w 0x00000601 -n ${num} -N ${num} -p 0x00001F07 -P 0x00001F07 -t 100000 >>  hdl_perf_test.log 2>&1"
    eval ${cmd}
    if [ $? -ne 0 ]; then
        echo "cmd: ${cmd}"
        echo "failed, please check hdl_perf_test.log"
...
rm -f hdl_perf_test.log
touch hdl_perf_test.log
...
./process_bandwidth.awk hdl_perf_test.log
./process_variance.awk hdl_perf_test.log
