...
static void rBurstOfDataMem(snap_membus_512_t *din_gmem,
					 snapu64_t input_address,
					 unsigned char *buffer)
{
	//#pragma HLS INLINE
	memcpy((char *)buffer, (snap_membus_512_t  *) (din_gmem + input_address), BPERDW_512);
}


// WRITE DATA TO MEMORY
static void wBurstOfDataMem(snap_membus_512_t *dout_gmem,
					 snapu64_t input_address,
					 unsigned char* buffer)
{
	//#pragma HLS INLINE
	memcpy((snap_membus_512_t *)(dout_gmem + input_address), (char *) buffer, BPERDW_512);
}
...
static void strmInWrite(hls::stream<unsigned char> &in_stream, snap_membus_512_t *din_gmem, action_reg *act_reg, uint64_t idx, uint32_t nbPixel )
{
...
static void  strmOutWrite(hls::stream<unsigned char> &out_stream, snap_membus_512_t *dout_gmem, action_reg *act_reg, uint64_t odx, uint32_t nbPixel )
{
	unsigned char dst[BPERDW_512];
	uint32_t nb, done;

	#pragma HLS INLINE  // dataflow
	nb    = act_reg->Data.out.size / BPERDW_512 ;
	L2:
	//#pragma HLS PIPELINE
	for ( int j = 0; j < nb; j ++) {
		L21:
		for ( int i = 0; i < BPERDW_512; i++ ) {
			#pragma HLS UNROLL factor 64
			done = j*BPERDW_512 + i;
...
int main(void)
{
	#define MEMORY_LINES BPERDW_512
    int rc = 0;
    unsigned int i;
    static snap_membus_512_t  din_gmem[BPERDW_512];
    static snap_membus_512_t  dout_gmem[BPERDW_512];
    int size, dsize;
    char *cve_out;
    ...
    act_reg.Data.in.size = dsize/BPERDW_512 * BPERDW_512 + BPERDW_512;
    act_reg.Data.in.type = SNAP_ADDRTYPE_HOST_DRAM;

    act_reg.Data.out.addr = 0;
    act_reg.Data.out.size = dsize/BPERDW_512 * BPERDW_512 + BPERDW_512;;
