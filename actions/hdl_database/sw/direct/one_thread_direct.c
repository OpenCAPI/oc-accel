...
void config_single_regex  (struct snap_card* h,
                   ...
                   size_t pkt_size,
                   size_t stat_size,
                   uint32_t* final_output_info_addr,
                   int eng_id)
{
    uint32_t reg_data;
    ...
    action_write (h, REG (ACTION_OUTPUT_INFO_ADDR_L, eng_id),
                  (uint32_t) (((uint64_t) (final_output_info_addr+eng_id*0x80)) & 0xffffffff));
    action_write (h, REG (ACTION_OUTPUT_INFO_ADDR_H, eng_id),
                  (uint32_t) ((((uint64_t) (final_output_info_addr+eng_id*0x80)) >> 32) & 0xffffffff));
    VERBOSE2 (" Write ACTION_OUTPUT_INFO_ADDR done! \n");
...
//{
//    uint32_t reg_data;
//    uint32_t done_data = 0;
//    int eng_id;
//
//    for(eng_id=0; eng_id<eng_num; eng_id++) {
//        // Start copying the pattern from host memory to card
//        action_write (h, REG (ACTION_CONTROL_L, eng_id), 0x00000001);
//        action_write (h, REG (ACTION_CONTROL_H, eng_id), 0x00000000);
//        VERBOSE2 (" Write engine %d ACTION_CONTROL for pattern copying! \n", eng_id);
//    }
//
//    do {
//        for(eng_id=0; eng_id<eng_num; eng_id++) {
...
//            }
//
//            done_data = done_data | ((reg_data & 0x00000001) << eng_id);
//            
//            VERBOSE3 ("Pattern Phase: polling eng %d Status reg with 0X%X\n", eng_id, reg_data);
//        }
//
//        // Status[0]
//        if ((done_data & 0x000000FF) == 0xFF) {
...
//        action_write (h, REG (ACTION_CONTROL_H, eng_id), 0x00000000);
//        VERBOSE1 (" Write engine %d ACTION_CONTROL for working! \n", eng_id);
//    }
//
//    do {
//
...
//            }
//
//            done = ((reg_data & 0x00000006) == 6);
//            done_data = (done_data | (done << eng_id));
//
//        }
//
//        if ((done_data & 0x000000FF) == 0xFF) {
...
//        action_write (h, REG (ACTION_CONTROL_L, eng_id), 0x00000000);
//        action_write (h, REG (ACTION_CONTROL_H, eng_id), 0x00000000);
//        VERBOSE2 (" Write engine %d ACTION_CONTROL for stop working! \n", eng_id);
//    }
//
//    return;
//}

void regex_work (struct snap_card* h,
                   void* patt_src_base,
                   void* pkt_src_base,
                   void* stat_dest_base,
                   size_t patt_size,
                   ...
                   int eng_num)
{
    ...
    for(eng_id=0; eng_id<eng_num; eng_id++) {
        // Flush rest data
        action_write (h, REG (ACTION_CONTROL_L, eng_id), 0x00000001);
        VERBOSE2 (" Write engine %d ACTION_CONTROL for engine to start work! \n", eng_id);
    }

    do {

        for(eng_id=0; eng_id<eng_num; eng_id++) {
            reg_data = *(final_output_info_addr+eng_id*0x80);

            // Status[23:8]
            if ((reg_data & 0x00FFFF00) != 0) {
                VERBOSE0 ("Error code got 0X%X\n", ((reg_data & 0x00FFFF00) >> 8));
                VERBOSE0 ("Eng %d final information for debug: 0X%X\n", eng_id, reg_data);
                exit (EXIT_FAILURE);
            }
            ...
            done = ((reg_data & 0x00000008) == 8);
            done_data = (done_data | (done << eng_id));
        }

        if ((done_data & 0x000000FF) == 0xFF) {
            VERBOSE0 ("All engine work done!\n");
            for(eng_id=0; eng_id<eng_num; eng_id++) {
                reg_data = *(final_output_info_addr+1+eng_id*0x80);

                VERBOSE0 ("Engine %d find 0X%X match!\n",eng_id, reg_data);
            }
            break;
        }
                   ...
                   int eng_id)
{
    ...
    final_output_info = alloc_mem (64, 1024); //512b aligned, 1KB space for up to 16 engine, 512b space for each engine
    memset (final_output_info, 0, 1024);
    ...
    action_write (h, REG (ACTION_OUTPUT_INFO_ADDR_L, eng_id),
                  (uint32_t) (((uint64_t) final_output_info) & 0xffffffff));
    action_write (h, REG (ACTION_OUTPUT_INFO_ADDR_H, eng_id),
                  (uint32_t) ((((uint64_t) final_output_info) >> 32) & 0xffffffff));
    VERBOSE2 (" Write ACTION_OUTPUT_INFO_ADDR done! \n");
    ...
    do {
        ...
        // Status[23:8]
        if (((*final_output_info) & 0x00FFFF00) != 0) {
            VERBOSE0 ("Eng %d Error code got 0X%X\n", eng_id, (((*final_output_info) & 0x00FFFF00) >> 8));
            exit (EXIT_FAILURE);
        }

        if (((*final_output_info) & 0x00000010) != 0) {
            VERBOSE0 ("Memory space for stat used up!\n");
            exit (EXIT_FAILURE);
        }

        // Status[0]
        if (((*final_output_info) & 0x00000008) == 8) {
            VERBOSE1 ("Eng %d work done!\n", eng_id);
            break;
        }
    } while (1);    
                           ...
                           int eng_num)
{
    ...
    VERBOSE0 ("Pattern src address is:0X%X\n", patt_src_base);
    VERBOSE0 ("Packet src address is:0X%X\n", pkt_src_base);
    VERBOSE0 ("Stat dest address is:0X%X\n", stat_dest_base);
    VERBOSE0 ("Pattern size is:0X%X\n", patt_size);
    VERBOSE0 ("Packet size is:0X%X\n", pkt_size);
    VERBOSE0 ("Stat size is:0X%X\n", stat_size);
    VERBOSE0 ("Final output info address is:0X%X\n", final_output_info);

    for(eng_id=0; eng_id<eng_num; eng_id++){
        config_single_regex (dnc, patt_src_base, pkt_src_base, stat_dest_base,
                      patt_size, pkt_size, stat_size, final_output_info, eng_id);
    }

    regex_work (dnc, patt_src_base, pkt_src_base, stat_dest_base,
                  patt_size, pkt_size, stat_size, final_output_info,  eng_num);
...
int main (int argc, char* argv[])
{
    ...
    void* stat_dest_base = alloc_mem (64, stat_size);

    ERROR_CHECK (regex_scan_one_thread (dn, timeout, patt_src_base, pkt_src_base, stat_dest_base,  patt_size, pkt_size, stat_size, num_eng_using));
