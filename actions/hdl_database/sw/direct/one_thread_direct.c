                   ...
                   int eng_num)
{
    ...
    for(eng_id=0; eng_id<eng_num; eng_id++) {
        ...
        action_write (h, REG (ACTION_CONTROL_H, eng_id), 0x00000000);
        VERBOSE2 (" Write engine %d ACTION_CONTROL for stat flushing! \n", eng_id);
    }
                           ...
                           int eng_num)
{
    ...
    pattern_copy_regex (dnc, patt_src_base, pkt_src_base, stat_dest_base, 
                  patt_size, pkt_size, stat_size, eng_num);

    engine_work_regex (dnc, patt_src_base, pkt_src_base, stat_dest_base,
                  patt_size, pkt_size, stat_size, eng_num);

    stat_flush_regex (dnc, patt_src_base, pkt_src_base, stat_dest_base,
                  patt_size, pkt_size, stat_size, eng_num);
