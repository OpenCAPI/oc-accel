                   ...
                   int job_id)
{
    ...
    *(cmpl_base + 1) = 0x0;
    ...
    VERBOSE0 ("Descriptor address = %p\n", dsc_base);
    VERBOSE0 ("Completion address = %p\n", cmpl_base);
    action_write (h, REG_MP_INIT_ADDR_LO, (uint32_t) (((uint64_t) dsc_base) & 0xffffffff));
    action_write (h, REG_MP_INIT_ADDR_HI, (uint32_t) ((((uint64_t) dsc_base) >> 32) & 0xffffffff));
                           ...
                           int eng_num)
{
    ...
    config_single_regex (dnc, patt_src_base, pkt_src_base, stat_dest_base,
                      patt_size, pkt_size, stat_size, final_output_info, job_id);
...
int string_match (int argc, char* argv[], int id)
{
    char device[64];
    ...
    pid = getpid();
    VERBOSE0 (log, "Print id is %d, pid is %d", id, pid);
    pasid = snap_action_get_pasid (dn);
    ...
    ERROR_CHECK (regex_scan (dn, timeout, patt_src_base, pkt_src_base, stat_dest_base,0,  patt_size, pkt_size, stat_size, job_num));
