...
synth_design -mode default -directive Default -fanout_limit 400 -fsm_extraction one_hot -resource_sharing off -shreg_min_size 5 -flatten_hierarchy rebuilt -keep_equivalent_registers -no_lc -top oc_fpga_top -part $fpga_part

puts [format "%-*s%-*s%-*s%-*s"  $widthCol1 "" $widthCol2 "" $widthCol3 "Writing $dcp_dir/$oc_fpga_static_synth_dcp" $widthCol4 "" ]
