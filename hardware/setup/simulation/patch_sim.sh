...
case $SIMULATOR in
  ...
  "irun")
    ...
    if [ -f ${OCACCEL_HARDWARE_ROOT}/sim/ies/run.f ]; then
      perl -i.ori -pe 'BEGIN{undef $/;} s/(^-makelib.*\n.*glbl.v.*\n.*endlib)//mg' ${OCACCEL_HARDWARE_ROOT}/sim/ies/run.f; # remove glbl.v from compile list
    fi
    ;;
  "xcelium")
    ...
    if [ -f ${OCACCEL_HARDWARE_ROOT}/sim/xcelium/run.f ]; then
      perl -i.ori -pe 'BEGIN{undef $/;} s/(^-makelib.*\n.*glbl.v.*\n.*endlib)//mg' ${OCACCEL_HARDWARE_ROOT}/sim/xcelium/run.f; # remove glbl.v from compile list
      perl -i.ori -pe 'BEGIN{undef $/;} s/(^-endlib.*\n^-makelib xcelium_lib\/.* \\\n)//mg' ${OCACCEL_HARDWARE_ROOT}/sim/xcelium/run.f; # merge everything to one lib
      perl -i.ori -pe 'BEGIN{undef $/;} s/^-makelib xcelium_lib\/xil_defaultlib/-makelib xcelium_lib\/work/mg' ${OCACCEL_HARDWARE_ROOT}/sim/xcelium/run.f; # set lib name to work in run.f
    fi
    ;;
  "questa"|"modelsim")
    sed -i "s/  simulate/# simulate/g"                   $1/$2 # run up to elaboration, skip execution
